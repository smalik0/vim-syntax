...
function! s:makeElement()
   ...
   public ArrayList<Circle> getCircles() {
      f b:html_mode && b:tagName =~? b:emptyTags
      if b:haveAtt == 0
   ...
   el
      ...
      if l:alone
         exe 'normal! o</ArrayList<Circle> circles = new ArrayList<Circle>();
  tart!
      el
         exe 'normal! atart 
      en
...
/ >
   p>ublic ArrayList<Circle> getCircles() {
   ...
   public ArrayList<ConvexPolygon> getConvexPolygons() {
      ...
      for (Shape s: shapes) {
         if (s.getClass() == ConvexPolygon.class) {
            polys.add((ConvexPolygon)s);
         }
      }
      return polys;
      for (Shape s: shapes) {
